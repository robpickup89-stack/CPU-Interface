...
namespace CPU_Interface
{
    public partial class Form1 : Form
    {
        ...
        private readonly Dictionary<string, string> cpuMessages = new Dictionary<string, string>()
        {
            ...
            // WRN CODES
            { "CFG:INVALID CONFIGURATION", "Configuration data invalid or changed. Reload configuration (RCP required)." },
            { "GCF:GREEN CONFLICT FAULT", "Conflicting green or green/yellow outputs detected." },
            { "GNC:GENERAL NON-COMPLIANCE", "Controller operating outside specification." },
            { "MSF:MAIN PROCESSOR FAULT", "Internal software or processor status fault." },
            { "REL:RELAY FAULT", "Signal relay control fault detected." },
            { "RLP:SECOND RED LAMP FAULT", "Second red lamp failure detected on a stream." },
            { "SAF:SAFETY CARD FAULT", "Fault reported by Safety Processor Card." },
            { "SCF:SPECIAL CONDITIONING FAULT", "Fault generated by special conditioning logic." },
            { "SSG:PEDESTRIAN SIGNAL FAULT", "Audible or tactile signal persistently active." },
            { "SAF:NO ERROR", "No safety card error present." },
            { "SAF:CONFIGURATION ERROR", "Safety card configuration error." },
            { "SAF:WATCHDOG TIMEOUT", "Safety processor watchdog timeout." },
            { "SAF:COMMUNICATION ERROR", "Communication failure with safety processor." },
            { "SAF:GROUP CARD COMMS ERROR", "Communication error with lamp switch (group) card." },
            { "SAF:TIMING ERROR", "Safety timing violation detected." },
            { "SAF:CPU OVERLOAD", "Safety processor overload detected." },
            { "SAF:ID ERROR", "Hardware or configuration ID mismatch." },
            { "SAF:MAINS FREQUENCY ERROR", "Mains frequency out of tolerance." },
            { "SAF:MAINS SUPPLY ERROR", "Mains supply failure detected." },
            { "SAF:12V SUPPLY ERROR", "12V supply rail fault." },
            { "SAF:5V SUPPLY ERROR", "5V supply rail fault." },
            { "SAF:YELLOW RELAY FAULT", "Yellow signal relay faulty." },
            { "SAF:RED/GREEN RELAY FAULT", "Red/Green relay faulty." },
            { "SAF:CYCLE TIME ERROR", "Cycle time violation detected." },
            { "SAF:GROUP CONFLICT ERROR", "Conflict detected within a signal group." },
            { "SAF:GROUP MIN TIME ERROR", "Minimum time violation in signal group." },
            { "SAF:GROUP MAX TIME ERROR", "Maximum time violation in signal group." },
            { "SAF:GROUP SEQUENCE ERROR", "Signal group sequence incorrect." },
            { "SAF:RED ANSWERING ERROR", "Red lamp feedback incorrect." },
            { "SAF:RED MIN TIME ERROR", "Red minimum time violation." },
            { "SAF:RED MAX TIME ERROR", "Red maximum time violation." },
            { "SAF:YELLOW ANSWERING ERROR", "Yellow lamp feedback incorrect." },
            { "SAF:YELLOW MIN TIME ERROR", "Yellow minimum time violation." },
            { "SAF:YELLOW MAX TIME ERROR", "Yellow maximum time violation." },
            { "SAF:GREEN ANSWERING ERROR", "Green lamp feedback incorrect." },
            { "SAF:GREEN MIN TIME ERROR", "Green minimum time violation." },
            { "SAF:GREEN MAX TIME ERROR", "Green maximum time violation." },
            { "SAF:RED FLASH ERROR", "Red flashing fault detected." },
            { "SAF:YELLOW FLASH ERROR", "Yellow flashing fault detected." },
            { "SAF:GREEN FLASH ERROR", "Green flashing fault detected." },
            { "SAF:UNKNOWN ERROR", "Unknown safety card error." },
            { "LMF:LAMP FAILURE", "One or more lamps failed on phase/aspect." },
            { "LMF:CONFLICT", "Lamp current conflict detected (extra current present)." },
            { "LMF:UNSTABLE", "Lamp monitoring toroid unstable." },
            { "LMF:MAINS UNSTABLE", "Mains voltage unstable (measured by safety card)." },
            { "LAO:LAMPS ALL OFF", "All signal lamps extinguished." },
            { "LAO:CLEARED", "Lamps all off condition cleared." },
            { "DFM:DETECTOR FAULT", "Detector fault monitoring triggered." },
            { "LFD:LRT DETECTOR FAILED", "LRT detector failure detected." },
            { "LFE:LRT EVENT FAILED", "Prepare, Demand, Stopline or Exit event failed." },
            { "LSE:LRT SUSPECT EVENT", "LRT event suspect count exceeded." },
            { "POWERFAIL:MAINS POWER FAILURE", "Loss of mains supply detected." },
            { "MAINS UNSTABLE:SUPPLY FLUCTUATION", "Mains voltage unstable." },
            { "MAINS CLEARED:SUPPLY RESTORED", "Mains voltage returned to normal." },
        ...
        private void FfsTimer_Tick(object? sender, EventArgs e)
        {
            ...
            // Check if WRN auto-scroll is enabled and active
            else if (wrnMode && chkFlfAutoScroll != null && chkFlfAutoScroll.Checked)
            {
                shouldSend = true;
            }
        ...
        private void HandleFfsResponse(string line)
        {
            ...
            // Check for WRN fault entry - need to send "+" until we see a repeat
            // WRN format: WRN CODE:DESCRIPTION
            bool isWrnEntry = upperLine.StartsWith("WRN") && upperLine.Contains(":");
            ...
            else if (isWrnEntry)
            {
                ...
                if (string.Equals(lastWrnEntry, upperLine, StringComparison.OrdinalIgnoreCase))
                {
                    wrnRepeatCount++;
                }
                else
                {
                    lastWrnEntry = upperLine;
                    wrnRepeatCount = 0;
                }
                ...
                if (wrnRepeatCount >= 1)
                {
                    wrnMode = false;
                    ffsTimer?.Stop();
                    textBox1.AppendText($"[WRN LOG COMPLETE - Repeat detected]{Environment.NewLine}");
                    lastWrnEntry = null;
                    wrnRepeatCount = 0;
                    return;
                }
        ...
        private string ExtractCodeKey(string line)
        {
            ...
            // 0c) Check for WRN format: WRN CODE:DESCRIPTION - Convert to CODE:DESCRIPTION for lookup
            if (line.StartsWith("WRN", StringComparison.OrdinalIgnoreCase))
            {
                string remainder = line.Substring(3).TrimStart();
                int dashIndex = remainder.IndexOf('–');
                if (dashIndex < 0)
                {
                    dashIndex = remainder.IndexOf('-');
                }
            ...
            // 1b) WRN style code formats e.g. CFG:INVALID CONFIGURATION
            var wrnStyleMatch = Regex.Match(line, @"^[A-Z0-9/ ]+:[A-Z0-9/ ]+");
            if (wrnStyleMatch.Success)
            {
                string candidate = wrnStyleMatch.Value.Trim();
                ...
                if (dashIndex > 0)
                {
                    candidate = line.Substring(0, dashIndex).Trim();
                }
